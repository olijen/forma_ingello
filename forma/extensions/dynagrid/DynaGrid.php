...
namespace forma\extensions\dynagrid;
...
class DynaGrid extends Widget
{
    ...
    public $editIcon = '<i class="glyphicon glyphicon-pencil"></i>';
    ...
    protected function setWidgetColumns()
    {
        ...
        $indicator = Html::tag('span', $this->editIcon, ['class' => 'rename-column-btn']);

        foreach ($this->_columns as $key => $column) {
            $order = ArrayHelper::getValue($column, 'order', self::ORDER_MIDDLE);
            $disabled = ($order == self::ORDER_MIDDLE) ? false : true;
            $widgetColumns = [
                'content' => (empty($indicator) || !isset($column['attribute']) ? '' : $indicator . ' ') .
                    Html::tag('span', $this->getColumnLabel($key, $column), [
                        'class' => 'config-column-label',
                    ]),
                'options' => ['id' => $key, 'data-data' => $column],
            ];
